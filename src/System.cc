...
cv::Mat System::TrackMonocular(const cv::Mat &im,const cv::Mat &iroi,const cv::Mat &iscore, const double &timestamp)
{
    if(mSensor!=MONOCULAR)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,iroi,iscore,timestamp);
